...
### 2) Draft article
- Use the Rake task to scaffold the post and image folder in one step:
  ```
  rake post TITLE="Your Post Title"
  ```
  This creates:
  - `_posts/YYYY-MM-DD-your-post-title.markdown` with full frontmatter
  - `assets/images/posts/your-post-title/index.html` to block directory browsing → 404
- Frontmatter minimum:
  - `layout: post`
  - `title`
  - `published: true`
...
`/llms.txt` and `/llms-full.txt` are auto-generated by Jekyll on every build — no manual updates needed. Every new post you publish automatically appears in both files.

- `/llms.txt` — index: title, URL, date, description for every post
- `/llms-full.txt` — full plain-text content of every post, stripped of HTML

These are Liquid templates in the root of the repo. Jekyll rebuilds them whenever `site.posts` changes, which happens automatically when a new post is added and the site is built.
